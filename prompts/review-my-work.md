...
Use the current Codex session context plus the caller's work report for this review.
Do not ask the caller to restate the full task background unless the review is impossible without it.
...
- if the reviewed work is acceptable but the larger agreed plan still has remaining steps, say clearly that this approval is only for the reviewed step and the caller should continue to the next agreed step instead of stopping
