...
Use this in **caller-mutates** mode after the caller has performed state-changing work and before the caller says done or performs commit/PR/merge/release/deploy.

The caller owns mutation. Codex reviews the caller's work, may do read-only investigation, and must not edit files or change project/external state in this call.
...
- When the caller's self-check found uncertainty or weak evidence

## Collaboration rules

- For a new shared task, or after compact/context clear, run `init.md` before work review.
- This call is a hard gate. The caller must not treat the work as accepted until Codex returns `approved_work: true`.
- `approved_work: true` accepts the reviewed step, not automatically the entire larger plan. If more agreed steps remain, the caller should continue directly to the next agreed step instead of stopping.
...
- If the caller disagrees with Codex's findings, switch to `chat.md` and discuss evidence until consensus or user escalation.
...
```json
{
  "work_for_review": "Describe the caller's actual work, validation, and remaining concerns here."
}
```
...
- `work_for_review` should include what the caller changed, what was verified, any known deviations from the earlier approved plan, and any remaining risks or uncertainty
...
- `approved_work: true` means the caller may treat the reviewed step as accepted; it does not by itself mean the whole larger plan is complete
- `approved_work: false` means the caller must not treat the work as accepted yet
